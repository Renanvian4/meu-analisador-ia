...
# --- DEFINIÇÃO DE ATIVOS EXPANDIDA ---
FUTUROS = ["WIN=F", "WDO=F", "BTC-USD", "ETH-USD", "SOL-USD", "XRP-USD", "DOGE-USD"]
B3 = ["PETR4.SA", "VALE3.SA", "ITUB4.SA", "BBDC4.SA", "ABEV3.SA"]
US_STOCKS = ["AAPL", "TSLA", "NVDA", "AMZN"]
OPCOES_ATIVOS = FUTUROS + B3 + US_STOCKS
...
# --- MOTOR DE APRENDIZADO NATIVO (CORRIGIDO) ---
    ...
    for ativo in selecionados:
        for tf_nome, periodo in timeframes.items():
            ...
            try:
                ...
                # Cálculo blindado para evitar ValueError (1000000234.png)
                close = hist['Close'].values.flatten()
                high = hist['High'].values.flatten()
                low = hist['Low'].values.flatten()
                open_p = hist['Open'].values.flatten()
                
                # RSI e Médias manuais para estabilidade
                ...
                pavio = high - low
                corpo = np.where(pavio > 0, np.abs(close - open_p) / pavio, 0)
                
                estudo = []
                for i in range(50, len(close) - 5):
                    if np.isnan(rsi[i]): continue
                    snap = [round(rsi[i], 2), round(dist[i], 4), round(corpo[i], 2)]
                    sucesso = 1 if close[i+5] > close[i] else 0
...
selecionados = st.sidebar.multiselect("Monitorar Agora:", OPCOES_ATIVOS, default=["WIN=F", "WDO=F"])
tf_operacional = st.sidebar.selectbox("Timeframe", ["1m", "5m", "15m", "1h"], index=1)

if st.sidebar.button("🧠 Deep Scan (Atualizar Memória)"):
    executar_deep_scan(selecionados)
    st.sidebar.success("Memória Multigráfico Atualizada!")
    ...
    while True:
        try:
            ...
            for ativo in selecionados:
                ...
                # Extração de dados atuais
                c = df['Close'].iloc[-1]
                ema = df['Close'].ewm(span=200, adjust=False).mean().iloc[-1]
                
                # Lógica simplificada de predição para o Tablet
            ...
            placeholder_logs.dataframe(pd.DataFrame(st.session_state.log_visual), use_container_width=True)
            time.sleep(20)
        except:
            time.sleep(10)
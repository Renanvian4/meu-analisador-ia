...
if busca_extra and busca_extra not in watchlist: watchlist.append(busca_extra)
...
# --- LAYOUT PRINCIPAL (FORÇAR RENDERIZAÇÃO) ---
...
# RESERVA OS ESPAÇOS NA TELA ANTES DO LOOP[cite: 7, 9]
    ...
    while True:
        try:
            for ativo in watchlist:
                # Download com período maior para garantir cálculos[cite: 7, 9]
                df = yf.download(ativo, period="7d", interval=tf_op, progress=False)
                if df.empty or len(df) < 50: continue
                if isinstance(df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
                
                # CÁLCULO DOS 4 INDICADORES REAIS
                ...
                if not df_clean.empty:
                    ...
                    prob_compra, prob_venda = preds[2], preds[1]
                    ...
                    if status != "⏳ Neutro":
                        ...
                        # Registra sinal único no minuto
                ...
                # ATUALIZAÇÃO VISUAL FORÇADA[cite: 9, 10]
